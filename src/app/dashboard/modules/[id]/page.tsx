...
export default function SecureModulePage() {
  ...
  useEffect(() => {
    if (!user) {
      router.push(`/login?redirect=${encodeURIComponent(`/dashboard/modules/${moduleId}`)}`);
      return;
    }
      ...
      <DashboardLayout>
        <div className="max-w-4xl mx-auto p-6">
          <div className="text-center py-16">
            ...
            <h2 className="text-lg font-medium text-white mb-2">Module not found</h2>
            <p className="text-sm text-gray-500 mb-6">The module you&apos;re looking for doesn&apos;t exist.</p>
            <button
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-5 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1.5">
                <h1 className="text-lg font-semibold text-white">{moduleData.name}</h1>
              ...
              <p className="text-xs text-gray-500">{moduleData.description}</p>
            </div>
          </div>
          ...
          <div className="mt-4 pt-4 border-t border-gray-800">
            <div className="flex items-center justify-between text-xs mb-2">
              ...
              <span className="text-white font-medium">{progress?.completionPercentage || 0}%</span>
            </div>
            ...
            <div className="flex items-center justify-between mt-2 text-[10px] text-gray-500">
              <span>{progress?.completedLessonCount || 0}/{moduleData.totalLessons} lessons</span>
              <span className="text-amber-400">{progress?.pointsEarned || 0} pts earned</span>
            </div>
          </div>
        </div>
        ...
        {!hasMoreLessons ? (
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-10 text-center">
            ...
            <h2 className="text-xl font-semibold text-white mb-2">Module Complete!</h2>
            <p className="text-sm text-gray-500 mb-3">You&apos;ve finished all lessons.</p>
            <p className="text-base font-medium text-amber-400 mb-6">{progress?.pointsEarned || 0} points earned</p>
            <button
              onClick={() => router.push("/dashboard/explore")}
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
            <div className={`p-8 text-center ${quizResults.quizResults.isPassed ? "bg-emerald-500/5" : "bg-rose-500/5"}`}>
              <div className={`w-14 h-14 mx-auto mb-4 rounded-full flex items-center justify-center ${quizResults.quizResults.isPassed ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-rose-500/10 border border-rose-500/20"}`}>
              ...
              <h2 className="text-lg font-semibold text-white mb-1">{quizResults.feedback}</h2>
              <p className="text-sm text-gray-400">
                Score: <span className="text-white font-medium">{quizResults.quizResults.earnedPoints}</span> / {quizResults.quizResults.totalPoints} points
              </p>
            </div>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="text-sm font-medium text-white mb-2">{result.question}</h4>
                        ...
                        {result.type === "subjective" && (
                          <div className="mb-2">
                            <p className="text-[10px] text-gray-500 mb-1">Your answer:</p>
                            <div className="bg-gray-950 text-gray-300 p-3 rounded-lg font-mono text-xs overflow-x-auto border border-gray-800">
                              <pre className="whitespace-pre-wrap">{result.userAnswer || "No answer"}</pre>
                            </div>
                          </div>
                        )}
            ...
            <div className="p-5 border-b border-gray-800">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">Test your knowledge</p>
                </div>
                    ...
                    <div className="mb-6">
                      <div className="flex items-start gap-3 mb-5">
                        ...
                        <div className="flex-1">
                          ...
                          {question.points && (
                            <p className="text-[10px] text-gray-500 mt-1">{question.points} points</p>
                          )}
                        </div>
                      </div>
                      ...
                      {question.type === "mcq" && question.options ? (
                        <div className="space-y-2">
                          ...
                          {question.options.map((option: any, index: number) => {
                            const isSelected = currentAnswer?.optionId === option._id;
                            return (
                              <button
                                key={option._id || option.id}
                                onClick={() => handleAnswerSelect(question._id, option._id || option.id)}
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  <div className={`flex-shrink-0 w-6 h-6 rounded-md flex items-center justify-center text-xs font-medium ${
                                    isSelected ? "bg-white text-black" : "bg-gray-800 text-gray-500"
                                  }`}>
                                    {String.fromCharCode(65 + index)}
                                  </div>
                                  <span className="text-sm text-gray-300">{option.text || option.content}</span>
                                </div>
                              </button>
                            );
                          })}
                        ...
                        <div>
                          {question.evaluationCriteria && (
                            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-3 mb-3">
                              <p className="text-xs text-gray-400">
                                <span className="font-medium text-gray-300">Criteria:</span> {question.evaluationCriteria}
                              </p>
                            </div>
                          )}
                          <div className="border border-gray-800 rounded-lg overflow-hidden">
                            <CodeEditor
                              ...
                              onChange={(value: string) => handleAnswerSelect(question._id, undefined, value)}
                              language="javascript"
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
            ...
            <div className="p-5 border-b border-gray-800">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div className="flex-1">
                  <h1 className="text-lg font-semibold text-white mb-2">{currentLesson.title}</h1>
            ...
            <div className="p-5 pt-0 flex items-center justify-between">
              ...
              {(currentLesson.type === "Code" || currentLesson.content?.includes("```") || currentLesson.content?.toLowerCase().includes("exercise")) && (
                <button
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-10 text-center">
            ...
            <h2 className="text-lg font-medium text-white mb-2">No Lesson Available</h2>
            <p className="text-sm text-gray-500 mb-6">Check back soon for new content.</p>
            <button
              onClick={() => router.push("/dashboard/explore")}
