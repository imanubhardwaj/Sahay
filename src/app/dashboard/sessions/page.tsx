                ...
                <div className="p-5">
                  <div className="flex items-start gap-4">
                      ...
                      <div className="mt-4 p-3 bg-gray-800/50 rounded-lg border border-gray-700/50">
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                          ...
                          <div>
                            ...
                            <span className="font-medium text-white">{booking.sessionTime}</span>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <span className="font-medium text-amber-400">{booking.price} pts</span>
                          </div>
                        </div>
                      </div>
                      ...
                      {booking.studentNotes && (
                        <div className="mt-3 p-3 bg-gray-800/30 rounded-lg border border-gray-700/50">
                          <p className="text-xs text-gray-400">
                            <span className="text-gray-300">Student notes:</span> {booking.studentNotes}
                          </p>
                        </div>
                      )}
                        ...
                        {booking.status === "completed" && booking.feedback && (
                          <div className="w-full p-3 bg-gray-800/50 rounded-lg border border-gray-700/50">
                                ...
                                {[1, 2, 3, 4, 5].map((star) => (
                                  <svg
                                    ...
                                    className={`w-3.5 h-3.5 ${
                                      star <= (booking.feedback?.studentRating || 0)
                                        ? "text-amber-400"
                                        : "text-gray-600"
                                    }`}
                                    fill="currentColor"
                            ...
                            {booking.feedback?.studentReview && (
                              <p className="text-xs text-gray-400">{booking.feedback.studentReview}</p>
                            )}
                          </div>
                        )}
                      </div>
                    ...
                    <div className="text-right hidden sm:block">
                      ...
                      <div className="text-base font-bold text-white">{booking.sessionTime}</div>
                      <div className="text-xs font-medium text-amber-400 mt-1">{booking.price} pts</div>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {showFeedbackModal && selectedBooking && (
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
            <div className="bg-gray-900 rounded-xl max-w-md w-full p-6 border border-gray-800">
              <h2 className="text-lg font-semibold text-white mb-1">Complete Session</h2>
              <p className="text-sm text-gray-500 mb-6">
                Session with {selectedBooking.studentId?.firstName} {selectedBooking.studentId?.lastName}
              </p>

              <div className="mb-5">
                <label className="block text-xs font-medium text-gray-400 mb-3">Rate the Student</label>
                    ...
                    >
                      <svg
                        className={`w-8 h-8 transition-colors ${
                          star <= feedback.rating ? "text-amber-400" : "text-gray-700"
                        }`}
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
              ...
              <div className="mb-6">
                <label className="block text-xs font-medium text-gray-400 mb-2">Feedback (Optional)</label>
                <textarea
