...
export default function MentorSchedulePage() {
    ...
    try {
      ...
      if (result.success && result.data) {
        setWeeklyAvailability(
          (prev) => result.data.defaultAvailability || prev
        );
  ...
  const fetchBookings = useCallback(async () => {
    try {
      const { getAuthHeaders } = await import("@/lib/token-storage");
      const response = await fetch(
        `/api/bookings?professionalId=${user?._id}`,
        {
          headers: getAuthHeaders(),
          credentials: "include",
        }
    ...
    Object.entries(weeklyAvailability).forEach(([, slots]) => {
      slots.forEach((slot) => {
        ...
        const slotsInRange = Math.floor(
          (endMinutes - startMinutes) / SESSION_DURATION
        );
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-6">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-white font-medium">
              Max {MAX_SLOTS_PER_DAY}/day
            </span>
            <span className="text-gray-600">·</span>
            <span className="text-white font-medium">
              Max {MAX_SLOTS_PER_WEEK}/week
            </span>
          </div>
        </div>
                ...
                {DAYS.map((day) => {
                  ...
                  return (
                    <div key={day.key} className="flex items-center gap-4">
                      ...
                      <button
                        onClick={() =>
                          toggleDayAvailability(day.key, !isEnabled)
                        }
                      ...
                      {isEnabled ? (
                        <div className="flex items-center gap-3 flex-1">
                          <input
                            ...
                            onChange={(e) =>
                              updateTimeSlot(day.key, "start", e.target.value)
                            }
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              updateTimeSlot(day.key, "end", e.target.value)
                            }
                            className="px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-white"
                          />
                          <span className="text-xs text-gray-500 ml-2">
                            {(() => {
                              const [sh, sm] = (daySlots[0]?.start || "09:00")
                                .split(":")
                                .map(Number);
                              const [eh, em] = (daySlots[0]?.end || "17:00")
                                .split(":")
                                .map(Number);
              ...
              {loading ? (
                ...
                <div className="p-12 text-center">
                  ...
                  <p className="text-gray-600 text-sm">
                    Create slots and share with students
                  </p>
                </div>
              ) : (
                        ...
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center text-white font-semibold">
                            {typeof booking.studentId === "object"
                              ? (
                                  booking.studentId as unknown as {
                                    firstName: string;
                                  }
