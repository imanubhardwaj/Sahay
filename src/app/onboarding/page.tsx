...
const getSectionsForUserType = (userType: string): Section[] => {
  const baseSections: Section[] = [
    {
      ...
      questions: [
        ...
        { id: 'location', label: 'Location', type: 'text', placeholder: 'City, Country', required: false },
      ]
    }
  ];
  ...
  if (userType === 'student_fresher') {
    baseSections[0].questions.push(
      { id: 'college', label: 'College/University', type: 'text', placeholder: 'Your institution name', required: true },
    ...
    baseSections[0].questions.push(
      ...
      { id: 'experience', label: 'Years of Experience', type: 'select', options: ['1-2 years', '3-4 years', '5-6 years', '7-8 years', '9+ years'], required: true }
    );
  }
  ...
  baseSections.push(
    {
      ...
      icon: '🎯',
      questions: [
        { id: 'domain', label: 'Primary Interest', type: 'select', options: ['Web Development', 'Mobile Development', 'Data Science', 'Machine Learning', 'DevOps', 'Cybersecurity'], required: true },
        { id: 'experience_level', label: 'Experience Level', type: 'select', options: ['Complete Beginner', 'Some Experience', 'Intermediate', 'Advanced'], required: true },
        { id: 'learning_goals', label: 'Learning Goals', type: 'multiselect', options: ['Get a Job', 'Build Projects', 'Learn New Skills', 'Career Change', 'Academic Excellence', 'Lead Teams', 'Start a Company'], required: true },
        { id: 'time_commitment', label: 'Time Commitment', type: 'select', options: ['1-2 hours/week', '3-5 hours/week', '6-10 hours/week', '10+ hours/week'], required: true },
        { id: 'preferred_learning_style', label: 'Learning Style', type: 'select', options: ['Visual (Videos, Diagrams)', 'Reading (Articles, Docs)', 'Hands-on (Projects)', 'Interactive (Quizzes)', 'Mixed Approach'], required: true },
      ]
    },
    {
      id: 'skills',
      ...
      icon: '🚀',
      questions: [
        { id: 'current_skills', label: 'Current Skills', type: 'multiselect', options: ['JavaScript', 'Python', 'Java', 'React', 'Node.js', 'SQL', 'Git', 'HTML/CSS', 'TypeScript', 'AWS', 'Docker', 'Kubernetes'], required: true },
        { id: 'interest_areas', label: 'Areas of Interest', type: 'multiselect', options: ['Frontend', 'Backend', 'Full Stack', 'Mobile Apps', 'AI/ML', 'Cloud Computing', 'Blockchain', 'Game Development', 'DevOps', 'Architecture'], required: true },
        { id: 'project_experience', label: 'Project Experience', type: 'select', options: ['No projects yet', '1-2 small projects', '3-5 projects', '5+ projects', 'Professional experience'], required: true },
        { id: 'career_goals', label: 'Career Goals', type: 'text', placeholder: 'Software Engineer, Data Scientist, Tech Lead, CTO, etc.', required: true },
        { id: 'challenges', label: 'Main Challenges', type: 'multiselect', options: ['Time Management', 'Staying Motivated', 'Finding Resources', 'Understanding Concepts', 'Building Projects', 'Leading Teams', 'Technical Strategy'], required: true },
      ]
    }
  );
...
export default function OnboardingPage() {
  ...
  const sections = selectedUserType ? getSectionsForUserType(selectedUserType) : [];
  const currentSectionData = sections[currentSection];
  const currentQuestionData = currentSectionData?.questions[currentQuestion];
  const totalQuestions = sections.reduce((sum, section) => sum + section.questions.length, 0);
  const currentQuestionIndex = sections.slice(0, currentSection).reduce((sum, section) => sum + section.questions.length, 0) + currentQuestion + 1;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const updateData = {
        ...
        experience: formData.experience ? parseInt(formData.experience.toString().split(' ')[0]) : undefined,
        ...
        yoe: formData.experience ? parseInt(formData.experience.toString().split(' ')[0]) : 0,
        ...
        <div className="w-full max-w-4xl">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-gray-600 mb-8">
              Let&apos;s personalize your learning experience. First, tell us who you are:
            </p>
          </div>
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-gray-200/50">
          ...
          <div className="mb-8">
            {currentQuestionData?.type === 'text' && (
              <Input
                value={formData[currentQuestionData.id as keyof OnboardingData] as string || ''}
                ...
                {currentQuestionData.options?.map((option: string) => (
                  <button
                    ...
                    className={`w-full p-4 text-left rounded-2xl border-2 transition-all duration-200 ${
                      formData[currentQuestionData.id as keyof OnboardingData] === option
            ...
            {currentQuestionData?.type === 'multiselect' && (
              <div className="space-y-3">
                {currentQuestionData.options?.map((option: string) => {
                  const selected = (formData[currentQuestionData.id as keyof OnboardingData] as string[] || []).includes(option);
                  return (
                    <button
                      key={option}
                      onClick={() => {
                        const current = formData[currentQuestionData.id as keyof OnboardingData] as string[] || [];
