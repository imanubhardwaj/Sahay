...
export async function GET(request: NextRequest) {
  try {
    ...
    const mentorProfile = await MentorProfile.findOne(query)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (cleanProfileData.sessionTypes && Array.isArray(cleanProfileData.sessionTypes)) {
      cleanProfileData.sessionTypes = cleanProfileData.sessionTypes.filter(session => 
    ...
    await mentorProfile.populate('userId', 'firstName lastName email avatar bio');

    return NextResponse.json({
      success: true,
      data: mentorProfile,
      message: mentorProfile ? 'Profile updated successfully' : 'Profile created successfully',
    });
  } catch (error) {
