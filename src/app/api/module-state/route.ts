...
export async function GET(request: NextRequest) {
  try {
    ...
    if (moduleProgress.nextLessonOrder < 1 || isNaN(moduleProgress.nextLessonOrder)) {
      moduleProgress.nextLessonOrder = 1;
      await moduleProgress.save();
    }

    // Get the module details
    const module = await Module.findById(moduleId).populate('skillId', 'name');
    ...
    const completionPercentage = validTotalLessons > 0 
      ? Math.round((moduleProgress.completedLessonCount / validTotalLessons) * 100)
      : 0;

    // Ensure completionPercentage is a valid number
    const validCompletionPercentage = isNaN(completionPercentage) ? 0 : completionPercentage;
    ...
    const finalCompletionPercentage = validCompletionPercentage === 0 && actualCompletedCount > 0
      ? Math.round((actualCompletedCount / validTotalLessons) * 100)
      : validCompletionPercentage;

    // Check if module is actually completed
    const isModuleCompleted = moduleProgress.completedLessonCount >= validTotalLessons && validTotalLessons > 0;
    ...
    if (!isModuleCompleted) {
      ...
      console.log('Looking for next uncompleted lesson starting from order:', lessonOrderToFetch);
      
      // Find the next uncompleted lesson
      for (let order = lessonOrderToFetch; order <= validTotalLessons; order++) {
        const lesson = await Lesson.findOne({
          moduleId,
          order: order
        }).select('_id name order type');
        
        if (lesson && !(moduleProgress.completedLessons?.includes(lesson._id) || false)) {
      ...
      if (!currentLesson) {
        console.log('No uncompleted lesson found from nextLessonOrder, checking from lesson 1');
