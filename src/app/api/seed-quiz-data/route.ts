...
export async function POST() {
  try {
    ...
    for (const lesson of lessons) {
      ...
      const moduleQuizData = quizData[module.name];
      if (!moduleQuizData) continue;

      const lessonQuizData = moduleQuizData.find(q => q.lessonOrder === lesson.order);
    ...
    const questionsWithQuizIds = questionsData.map(questionData => {
      const quiz = quizzes.find(q => 
        q.lessonId.toString() === questionData.lessonId.toString()
      );
      return {
        ...questionData,
        quizId: quiz?._id
