...
export async function GET(request: NextRequest) {
  try {
    ...
    const userId = searchParams.get('userId') || request.cookies.get('user_id')?.value;
    ...
    let userProgress = await UserCourseProgress.findOne({ userId: userObjectId });
    ...
    if (!userProgress) {
      ...
      console.log('Found existing user progress with', userProgress.completedCourses?.length || 0, 'courses');
    }
    ...
    try {
      ...
      if (userProgress.completedCourses.some(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (c: any) => c.courseId)) {
        await userProgress.populate('completedCourses.courseId', 'name description level duration');
      }
      await userProgress.populate('completedCourses.moduleId', 'name');
      await userProgress.populate('completedCourses.currentLessonId', 'name order');
    } catch (populateError) {
    ...
    if (courseId) {
      const courseProgress = userProgress.completedCourses && userProgress.completedCourses.length > 0 
        ? userProgress.completedCourses.find(
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (c: any) => c.courseId && c.courseId._id.toString() === courseId
          )
        : null;
      return NextResponse.json({ courseProgress: courseProgress || null }, { status: 200 });
    }

    // If moduleId is provided, filter to that module
    if (moduleId) {
      const moduleProgress = userProgress.completedCourses && userProgress.completedCourses.length > 0
        ? userProgress.completedCourses.find(
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (c: any) => c.moduleId && c.moduleId._id.toString() === moduleId
          )
        : null;
      return NextResponse.json({ moduleProgress: moduleProgress || null }, { status: 200 });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const userId = bodyUserId || request.cookies.get('user_id')?.value;
    ...
    if (!userProgress) {
      console.log('Creating new user progress for POST request, userId:', userId);
      ...
      console.log('Found existing user progress for POST, courses:', userProgress.completedCourses?.length || 0);
    }
    ...
    let courseProgress = userProgress.completedCourses && userProgress.completedCourses.length > 0
      ? userProgress.completedCourses.find(
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    if (courseProgress) {
      ...
      courseProgress = {
        ...
        currentLessonId: lessonId ? new mongoose.Types.ObjectId(lessonId) : firstLesson._id,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Find user progress
    const userProgress = await UserCourseProgress.findOne({ userId: userObjectId });
    ...
    if (!courseProgress.completedLessonIds.some(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    return NextResponse.json({ 
      userProgress,
      courseProgress,
      nextLesson: nextLesson ? { _id: nextLesson._id, name: nextLesson.name } : null,
      message: courseProgress.status === 'completed' 
