...
export async function POST(request: NextRequest) {
  try {
    ...
    const moduleProgress = await ModuleProgress.findOne({ userId, moduleId }).read('primary').exec();
    ...
    if (!isLessonAlreadyCompleted) {
      ...
      pointsAwarded = lessonProgress?.pointsEarned || lesson.points || 0;
    ...
    moduleProgress.completionPercentage = isNaN(completionPercentage) ? 0 : completionPercentage;
    ...
    const response = {
      ...
      message: moduleProgress.status === 'completed' 
