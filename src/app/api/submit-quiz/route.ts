...
export async function POST(request: NextRequest) {
  ...
  try {
      ...
      quizQuestions.map(async (question) => {
        ...
        if (!userAnswer) {
          return {
            ...
            suggestedTopics:
              question.type === "mcq" ? lessonTopics.slice(0, 2) : [],
          };
        }
        ...
        if (question.type === "subjective" || question.type === "code") {
          try {
            const evaluation = await evaluateSubjectiveOrCode(
              ...
              question.type as "subjective" | "code",
              question.points || 10,
            );
        ...
        // If we get here, the question type didn't match any known type
        console.warn(
          `[SUBMIT-QUIZ] ⚠️ Unknown question type: "${question.type}"`,
        );
        console.warn(
          `[SUBMIT-QUIZ] ⚠️ Expected: "mcq", "subjective", or "code"`,
        );
        console.warn(`[SUBMIT-QUIZ] ⚠️ Question ID: ${question._id}`);

        // Return default result for unknown types
        return {
          questionId: question._id,
          question: question.questionText || question.question,
          type: question.type,
          ...
          earnedPoints: 0,
          isCorrect: false,
          userAnswer: userAnswer?.content || null,
    ...
    const response = {
      ...
      feedback: isPassed
        ? "🎉 Congratulations! You passed the quiz with " +
          scorePercentage +
          "%!"
