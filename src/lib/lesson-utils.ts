 ...
 */
export function extractCodingChallenge(content: string): CodingChallenge | null {
  if (!content) return null;

  // Find the Coding Challenge section
  const challengeMatch = content.match(/## Coding Challenge\s*\n([\s\S]*?)(?=\n## |$)/i);
  if (!challengeMatch) return null;

  const challengeSection = challengeMatch[1];
  
  // Extract Task 1
  const task1Match = challengeSection.match(/\*\*Task 1\*\*:\s*(.+?)(?=\n\*\*Task 2\*\*:|$)/is);
  const task1 = task1Match ? task1Match[1].trim() : undefined;

  // Extract Task 2
  const task2Match = challengeSection.match(/\*\*Task 2\*\*:\s*(.+?)(?=\n\*\*Task |$)/is);
 ...
 */
export function getModuleLanguage(moduleName: string): "javascript" | "python" | "typescript" {
 ...
 */
export function createProblemDescription(challenge: CodingChallenge, lessonTitle: string): string {
